...
mongoose.connect("mongodb://localhost/cs142project8", {
    ...
    var allPromises = Promise.all(userPromises).then(function () {
      ...
      for (var i = 0; i < userIDs.length; i++) {
        photoModels = photoModels.concat(
          cs142models.photoOfUserModel(userIDs[i])
        );
      }
      var photoPromises = photoModels.map(function (photo) {
        return Photo.create({
          ...
          .then(function (photoObj) {
            ...
            if (photo.likes) {
              const mappedLikes = photo.likes.map(user_id => mapFakeId2RealId[user_id]);
              photoObj.likes = mappedLikes;
            }
            if (photo.comments) {
